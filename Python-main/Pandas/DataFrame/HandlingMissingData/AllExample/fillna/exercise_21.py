#How can you fill missing values in the Name column with values from the previous row, but only for rows where Gender is 'F'
import pandas as pd
import numpy as np
...
data = {
    ...
    'Gender': ['F', 'M', 'M', 'F', 'F', 'M', np.nan, 'F', 'M', 'F'],
...
# df.loc[df['Gender'] == 'Female', 'Name'] = df.loc[df['Gender'] == 'Female', 'Name'].fillna(method='ffill')

df.loc[row,'Name'] = df.loc[row,'Name'].ffill()
