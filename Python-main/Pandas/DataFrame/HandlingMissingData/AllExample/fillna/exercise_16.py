#How would you fill missing values in the Salary column with the mean value of Salary for each Department?
import pandas as pd
import numpy as np
...
# df['Salary'] = df.apply(lambda row: df[df['Department'] == row['Department']]['Salary'].mean() if pd.isnull(row['Salary']) else row['Salary'], axis=1)

# Calculate the mean Salary for each Department
mean_salary_by_dept = df.groupby('Department')['Salary'].transform('mean')

# Fill missing values in Salary with the mean Salary for each Department
df['Salary'] = df['Salary'].fillna(mean_salary_by_dept)
