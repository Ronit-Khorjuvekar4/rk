# How would you fill missing values in the Education column using a forward fill method, but only for rows where Gender is female?
import pandas as pd
import numpy as np
...
data = {
    ...
    'Education': ['Bachelors', 'Masters', 'Bachelors', 'PhD', 'Masters', 'PhD', np.nan, 'Bachelors', 'Masters', np.nan]
}

df = pd.DataFrame(data)

ok = df.loc[df['Gender'] == 'F','Education']

df.loc[df['Gender'] == 'F','Education'] = ok.fillna(method='ffill')
