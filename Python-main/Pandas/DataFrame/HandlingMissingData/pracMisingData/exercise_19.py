# How can you replace missing values in the City column with a default value only for rows where Age is greater than 30?
import pandas as pd
import numpy as np
...
data = {
    ...
    'City': ['New York', 'Los Angeles', np.nan, np.nan, 'Phoenix', 'Philadelphia', 'San Antonio', 'San Diego', 'Dallas', np.nan],
    'Education': ['Bachelors', 'Masters', 'Bachelors', 'PhD', 'Masters', 'PhD', np.nan, 'Bachelors', 'Masters', 'PhD']
}

df = pd.DataFrame(data)

df.loc[df['Age'] > 30,'City'] = df.loc[df['Age'] > 30,'City'].fillna('DefaultValue')
