# How would you fill missing values in the Salary column with the mean of that column, but only if the ExperienceYears is greater than 5?
import pandas as pd
import numpy as np
...
ok = df.loc[df['ExperienceYears'] > 5,'Salary']

df.loc[df['ExperienceYears'] > 5,'Salary'] = ok.fillna(df['Salary'].mean())
