# What command would you use to drop rows that have missing values in at least 3 different columns?
import pandas as pd
import numpy as np
...
data = {
    ...
    'Age': [25, 28, 30, 35, 22, np.nan, 26, 29, np.nan,21],
    'ExperienceYears': [5, np.nan, 3, 7, 1, 15, 2, 6, 10, np.nan],
    'Gender': ['F', 'M', 'M', 'M', 'F', 'M', np.nan, 'F', 'M', np.nan],
...
df_dropped_rows = df.dropna(thresh=len(df.columns) - 2)
