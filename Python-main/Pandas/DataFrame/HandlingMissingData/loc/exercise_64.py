# Using loc, fill missing values in the JoiningYear 
# with the previous row's value, but only if the ExperienceYears is greater than 10.
#
import pandas as pd
import numpy as np
...
data = {
    ...
    'ExperienceYears': [5, np.nan, 3, 7, 11, 15, 2, 6, 10, 4, 8, np.nan, 9, 7, np.nan, 5, 12, np.nan, 8, 6, 14, 4, np.nan, 11, 10, 9, np.nan, 6, 5, 13],
...
print(df.loc[(df['JoiningYear'].isna()) & (df['ExperienceYears'] > 10),'JoiningYear'])

df.loc[(df['JoiningYear'].isna()) & (df['ExperienceYears'] > 10),'JoiningYear'] = df['JoiningYear'].ffill()
