# Select the 'Name' and 'Salary' of employees whose 'Salary' is within 5% of the mean salary of their department.
import pandas as pd
...
mean_salaries = df.groupby('Department')['Salary'].transform('mean')
std_salaries = df.groupby('Department')['Salary'].transform('std')
lower_bound = mean_salaries * 0.95
upper_bound = mean_salaries * 1.05
df.loc[(df['Salary'] >= lower_bound) & (df['Salary'] <= upper_bound), ['Name', 'Salary']]
print(df)
