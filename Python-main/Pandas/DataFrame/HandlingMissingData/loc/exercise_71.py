# Use loc to replace missing values in the Salary column 
# with the product of ExperienceYears and 2000, but only if Age is less than 30.
#
import pandas as pd
import numpy as np
...
data = {
    ...
    'Salary': [None, 60000, np.nan, 70000, 52000, np.nan, 62000, 50000, 61000, 70000, 58000, 64000, 72000, 52000, np.nan, 66000, 63000, 54000, 71000, 69000, 60000, np.nan, 57000, 65000, 55000, 68000, np.nan, 62000, 64000, 70000],
    ...
    'Age': [24, 28, 30, 35, 22, np.nan, 26, 29, np.nan, 27, 24, 33, 31, np.nan, 26, 40, 28, 34, 32, 29, np.nan, 37, 22, 30, 25, 35, 28, 33, np.nan, 26],
...
# Using loc
df.loc[df['Salary'].isna() & (df['Age'] < 30), 'Salary'] = df['ExperienceYears'] * 2000
