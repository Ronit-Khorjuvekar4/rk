# Update the City column to fill missing values
# using forward fill within each Department group, using loc.
import pandas as pd
import numpy as np
...
data = {
    ...
    'Department': ['HR', 'IT', 'Finance', 'IT', 'HR', 'Finance', np.nan, 'HR', 'Finance', 'IT', 'HR', 'Finance', 'IT', 'HR', np.nan, 'Finance', 'IT', 'HR', 'Finance', 'IT', 'Finance', np.nan, 'HR', 'IT', 'Finance', 'IT', 'HR', 'HR', 'Finance', 'IT'],
...
# Using loc
df.loc[:, 'City'] = df.groupby('Department', group_keys=False)['City'].apply(lambda x: x.ffill())

# Using standard DataFrame method
# df['City'] = df.groupby('Department', group_keys=False)['City'].ffill()
