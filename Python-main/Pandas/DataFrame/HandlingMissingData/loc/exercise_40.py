# Select the 'Name' and 'Salary' of employees in the 'Marketing' 
# department where their 'Salary' is in the top 10% of the department.
# 
import pandas as pd
...
row = (df['Department'] == 'Marketing')
ok = df.groupby('Department')['Salary'].sum()
temPercent = ok['Marketing'] * 0.1
ans = row.loc[row]

print(df.loc[(row) & (ans),['Name', 'Salary']])
