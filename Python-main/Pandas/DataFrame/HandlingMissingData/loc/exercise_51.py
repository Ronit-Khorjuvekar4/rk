# Using loc, replace all missing values in the Education column 
# with the most frequent Education within each Gender group.
#
import pandas as pd
import numpy as np
...
data = {
    ...
    'Gender': ['F', 'M', 'M', 'M', 'F', 'M', np.nan, 'F', 'M', 'F', 'M', 'F', np.nan, 'M', 'F', 'M', 'F', 'M', 'M', 'F', np.nan, 'F', 'M', 'F', 'M', 'F', 'F', 'M', 'F', 'M'],
...
# print(df.loc[df['Education'].isna(),['Education']])

# df.loc[:, 'Education'] = df.groupby('Gender',group_keys=False)['Education'].apply(lambda x: x.fillna(x.mode()[0]))
