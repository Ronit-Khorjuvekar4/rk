# Create a new column PerformanceBonus which is 
# 15% of Salary for employees in 'IT' whose Age is greater than 30 and 8% for others.
#
import pandas as pd
import numpy as np
data = {
...
df.loc[:,'PerformanceBonus'] = np.where((df['Department'] == 'IT') & (df['Age'] > 30),(df['Salary'] * 100) / 15,(df['Salary'] * 100) / 8 )
