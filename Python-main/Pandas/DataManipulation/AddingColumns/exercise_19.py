# Insert a new column NextPromotionYear which is JoiningYear plus 5 for employees whose Department is 'IT' and 6 for others.
#
import pandas as pd
import numpy as np
...
# df.loc[:,'NextPromotionYear'] = df.apply(lambda x: x['JoiningYear'] + 5 if x['Department'] == 'IT' else x['JoiningYear'] + 6, axis=1)

df.loc[:,'NextPromotionYear'] = np.where(df['Department'] == 'IT',df['JoiningYear'] + 5,df['JoiningYear'] + 6)

# conditions = [df['JoiningYear'] > 2018, df['JoiningYear'] <= 2018]
# choices = [df['Age'] + 1, df['Age'] + 2]
# df['AgeIn2025'] = np.select(conditions, choices)
